...
\begin{document}
    ...
    \begin{description}
        ...
        \item[3]{Sol}
            \begin{enumerate}
                \item [(a)]{WhileFun}\\
                    $\text{letrec}\: whileFun = \\ \text{function }t{_1}:(True \rightarrow Bool) \longrightarrow \text{function }t{_2}:(True -> True)  \text{ in}\\ \, \text{if } t_1 \; \text{then } t_2; whileFun \;\; t_1\,t_2 \text{ else} ()$
            \end{enumerate}
        \item[(4)]{Sol}
            \begin{enumerate}
                \item[(a)]{Small-Step}\\
...
| Node of 'a * 'a tree * 'a tree
        ...
        \item[(6)]{Sol} \\
            ...
            $\frac{}{\text{infer}(\Gamma,)}$
            \inferrule{I-match}
            {fresh X${_1}$, X${_2}$ \andalso tinfer($\Gamma$,s) = (S, C) \andalso tinfer($\Gamma , \text{left } x{_1} = C{_l})$ \andalso tinfer($\Gamma , \text{right } x{_2} = C{_r})$  \andalso tinfer($\{ \Gamma, x{_1}:X{_1} \} , s_{_1}$) = $(S{_1}, C{_1})$ \andalso tinfer($\{\Gamma, x{_2}:X{_2}\},s_{2}$) = ($S{_2},C{_2}$)}
            {tinfer($\Gamma$,match s with left x${_1}$ $\rightarrow$ s${_1}$ $|$ right x${_2}$ $\rightarrow$ s${_2}$)} = $(X, C \cup C{_1} \cup C{_2} \cup \{ S{_1} = S{_2}, S = S{_1} \vee S{_2} \} )$
    \end{description}
